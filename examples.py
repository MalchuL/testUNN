...
if __name__ == '__main__':
    data = ImageFolder(root='./dataset/images', transform=tr.Compose([tr.Resize((128,128)), ToTensor()]))
    masks = ImageFolder(root='./dataset/masks', transform=tr.Compose([tr.Resize((128,128)),tr.Grayscale(), ToTensor()]))
    all_data = []
    all_masks = []
    for img,_ in data:
        all_data.append(img)
